...
if test ! -n "$IOS_PROVISIONING_PROFILE" && test ! -f "$IOS_PROVISIONING_PROFILE"; then
  echo "HOOK >> Provisioning profile's file is missing! (path: $IOS_PROVISIONING_PROFILE) Xcode will find matching profile on his own."
else
...
echo "HOOK >> iOS platform has been configured to sign the app with key '$IOS_CODE_SIGN_IDENTITY.'"
